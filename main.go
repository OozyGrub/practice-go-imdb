...
func updateMoviesHandler(c echo.Context) error {
	...
	stmt, err := db.Prepare(`
	UPDATE goimdb
	SET imdbID=?,title=?, year=?, rating=?, isSuperHero=?
	WHERE imdbID=?
	`)
	...
	_, err = stmt.Exec(m.ImdbID, m.Title, m.Year, m.Rating, fmt.Sprintf("%v", m.IsSuperHero), imdbID)
	...
	rowx := db.QueryRow(`
		...
		FROM goimdb WHERE imdbID=?`, m.ImdbID)
